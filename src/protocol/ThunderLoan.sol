...
contract ThunderLoan is Initializable, OwnableUpgradeable, UUPSUpgradeable, OracleUpgradeable {
    ...
    /*//////////////////////////////////////////////////////////////
                            STATE VARIABLES
    //////////////////////////////////////////////////////////////*/
    mapping(IERC20 => AssetToken) public s_tokenToAssetToken; // e I think this maps the underlying token to its
        // assetToken. If a liquidity provider deposit USDC the USDC will mint AssetToken.

    // The fee in WEI, it should have 18 decimals. Each flash loan takes a flat fee of the token price.
    // @audit -info this should be constant or immutable.
    uint256 private s_feePrecision; // q why is this a storage variable?? it is initialized with figures under the
        // initialize contract because the constructor here is does not fo that because it is an upgradable SC.
    uint256 private s_flashLoanFee; // 0.3% ETH fee

    mapping(IERC20 token => bool currentlyFlashLoaning) private s_currentlyFlashLoaning; // e a mapping that tells us if
        // a token is in the middle of aflash loan.
    ...
    /*//////////////////////////////////////////////////////////////
                           EXTERNAL FUNCTIONS
    //////////////////////////////////////////////////////////////*/
    // @audit info change name to poolFactoryAddess
    ...
    function initialize(address tswapAddress) external initializer {
        ...
        __Oracle_init(tswapAddress); // e using tswap as some tiype of oracle.
    ...
    function deposit(IERC20 token, uint256 amount) external revertIfZero(amount) revertIfNotAllowedToken(token) {
        AssetToken assetToken = s_tokenToAssetToken[token]; //e represents the shares of liquidity providers in the
            // pool.
        uint256 exchangeRate = assetToken.getExchangeRate();
        // e 100e18 * 1e18 / 1e18(2e18)
        ...
        // e when the LP deposits, the money sits in the assetToken contract.
    ...
    {
        AssetToken assetToken = s_tokenToAssetToken[token];
        // e probably what we are gonna use to check if the flashloan has been repaid.
        ...
        //@audit messed up the slitther disables
        ...
        // ending balance >= starting balance + fee
    ...
    // e this is whatthe contract expect users to repay using
    // e users could just call tranfer
    // @audit -low you cannot use repay to repay a flash loan. inside another flashloan
    ...
    function setAllowedToken(IERC20 token, bool allowed) external onlyOwner returns (AssetToken) {
        if (allowed) {
            ...
            string memory name = string.concat("ThunderLoan ", IERC20Metadata(address(token)).name()); // the name will
                // appear as: ThunderLoan USDC
            string memory symbol = string.concat("tl", IERC20Metadata(address(token)).symbol()); // Symbol of assetToken
                // will come as: tlUSDC
            ...
            AssetToken assetToken = s_tokenToAssetToken[token];
            // q does deleting a mapping work right??
            // a yes!
    ...
    // is this calculating the fees of flashloans
    // a yes!
